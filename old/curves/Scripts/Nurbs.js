...
export class Nurbs {
    ...
    constructor(control_points) {
        ...
        this.weights = new Array(); //TODO make private and add interface
    ...
    eval_dp(beg, end, depth) {
        const a = this.rational_bspline_point(beg);
        const b = this.rational_bspline_point(end);
    ...
    rational_bspline_point(t) {
        let x = 0;
        let y = 0;
        let rationalWeight = 0;
        for (let i = 0; i < this.cp.size(); i++) {
            const temp = this.Nip(i, t) * this.weights[i];
            rationalWeight += temp;
        }
        for (let i = 0; i < this.cp.size(); i++) {
            const temp = this.Nip(i, t);
            x += this.get_cp(i).x * this.weights[i] * temp / rationalWeight;
            y += this.get_cp(i).y * this.weights[i] * temp / rationalWeight;
        }
